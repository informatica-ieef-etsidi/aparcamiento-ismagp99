...
void main() {
	...
	int i, j, k; //Para bucles, contadores y demás utilidades
	char acceso[50]; //Registra el usuario y después la contraseña introducidos
	...
	for (i = 1; i < 4; i++) { //Ofrece 3 intentos para introducir usuario y contraseña
		...
		if (j < AUTORIZACIONES) { //Compara contraseña, solo la asociada al usuario introducido previamente
			printf("\nIntroduzca su contrasena: ");
			gets(acceso);
			if (strcmp(acceso, autorizados[j].contraseña) == 0) {
				break;
			}
			else {
			...
			case 'c':
				...
				if (j == NPLAZASC) {
					...
					if (j != NPLAZAS) { //Impide que se repitan matrículas
						...
						if (k != 7) { //Si el contador k no valida los 7 caracteres se rechaza la matrícula
